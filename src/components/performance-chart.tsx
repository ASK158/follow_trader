    ...
    <div className={compact ? "chart chart-compact" : "chart chart-performance"}>
      ...
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ top: 18, right: compact ? 4 : 18, left: compact ? -28 : 14, bottom: 0 }}>
          ...
          <XAxis dataKey="date" tickLine={false} axisLine={false} tick={{ fill: "#8b98aa", fontSize: compact ? 10 : 12 }} tickFormatter={(value: string) => new Intl.DateTimeFormat("zh-CN", { month: "short", year: compact ? undefined : "2-digit" }).format(new Date(value))} minTickGap={compact ? 42 : 68} />
          {(compact || visibleSeries.balance) && <YAxis yAxisId={compact ? "growth" : "balance"} tickLine={false} axisLine={false} width={compact ? 36 : 72} tick={{ fill: compact ? "#4f8df7" : "#67a98b", fontSize: compact ? 10 : 12 }} tickFormatter={(value: number) => compact ? `${value}%` : currency.format(value)} />}
          {!compact && visibleSeries.growth && <YAxis yAxisId="growth" orientation="right" tickLine={false} axisLine={false} width={53} tick={{ fill: "#4f8df7", fontSize: 12 }} tickFormatter={(value: number) => `${value}%`} />}
          ...
          {(compact || visibleSeries.growth) && <Area yAxisId="growth" type="monotone" dataKey="growth" name="收益率" stroke="#4f8df7" strokeWidth={2.6} fill={compact ? "url(#fill-growth)" : "none"} dot={false} activeDot={{ r: 4 }} />}
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
