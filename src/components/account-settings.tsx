...
export function AccountSettings({ user, sessions: initialSessions }: Props) {
  ...
  return <div className="dev-product-list">
    ...
    <section className="dev-auth-card"><h2>账户身份</h2><p>{user.email} · {user.emailVerified ? "邮箱已验证" : "邮箱未验证"} · {user.role === "admin" ? "管理员" : "用户"}</p><Link href="/account/profile" className="dev-secondary-action">编辑头像、昵称和公开资料</Link></section>
