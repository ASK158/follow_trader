  ...
  return (
      <div className="details-flow">
        <section className="chart-panel performance-panel" aria-labelledby="performance-heading">
          <div className="panel-heading"><div><span className="panel-label">账户表现</span><h2 id="performance-heading">计收益曲线</h2><p className="muted">右上角可独立显示或隐藏曲线；资金使用左轴（{signal.currency}），收益率使用右轴（%）。</p></div><div className="chart-summary"><span>结余 <b>{currency.format(signal.balance)}</b></span><span>收益 <b>+{percent.format(signal.growth)}%</b></span></div></div>
          <PerformanceChart data={signal.curve} currency={signal.currency} />
        </section>

        <section className="panel-only" aria-labelledby="returns-heading">
          <div className="panel-heading"><div><span className="panel-label">收益率</span><h2 id="returns-heading">月度表现</h2></div><span className="muted">单位：%</span></div>
        ...
        <section className="panel-only history-panel" aria-labelledby="history-heading">
          <div className="panel-heading"><div><span className="panel-label">交易历史记录</span><h2 id="history-heading">已平仓交易</h2></div><span className="muted">每页 20 笔 · 已导入 {signal.tradesHistory.length} 笔<span className="trade-scroll-hint"> · 左右滑动查看完整字段</span></span></div>
