...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return tutorial ? { title: `${tutorial.title} | Sigma Signal 教程`, description: tutorial.summary } : {};
}
...
export default async function TutorialDetailPage({ params }: Props) {
  ...
  return (
    <main className="platform-shell tutorial-detail-shell">
      ...
      <footer className="platform-footer">教程内容仅用于技术学习，不构成交易建议。使用任何策略前请独立验证。</footer>
    </main>
  );
}
