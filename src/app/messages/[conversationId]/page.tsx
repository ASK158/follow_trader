...
export default async function ConversationPage({ params }: Props) {
  ...
  return <main className="platform-shell developer-shell"><SiteNav active="developer" /><div className="platform-breadcrumb"><Link href="/messages">← 返回消息</Link><span>DIRECT MESSAGE</span></div><header className="message-thread-header"><UserAvatar name={conversation.participant.participantName} src={conversation.participant.participantAvatarUrl} size={58} /><div><h1>{conversation.participant.participantName}</h1><Link href={`/u/${conversation.participant.participantUsername}`}>@{conversation.participant.participantUsername}</Link></div></header><DirectMessageThread conversationId={conversationId} currentUserId={user.id} initialMessages={conversation.messages} /><SiteFooter notice="请勿通过消息发送交易密码、API 密钥或支付凭据。" /></main>;
}
